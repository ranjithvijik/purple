...
html_to_pptx.py
---------------
Converts a multi-slide HTML presentation into a PowerPoint (.pptx) file.
...
PPTX_OUT   = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("pres_output/presentation.pptx")
SLIDE_W    = 1280   
SLIDE_H    = 720
SCALE      = 2      
JPEG_Q     = 95     
...
def build_pptx(slide_imgs: list[Path], pptx_path: Path) -> None:
    if not slide_imgs:
    ...
    print(f"Building PPTX ({len(slide_imgs)} slides) -> {pptx_path} ...")
    ...
    size_mb = pptx_path.stat().st_size / 1_048_576
    print(f"✓ Done: {pptx_path.name}  |  {size_mb:.1f} MB  |  {len(slide_imgs)} slides")
...
def main():
    ...
    tmp_dir = PPTX_OUT.parent / "_slides_tmp"
    ...
    try:
        build_pptx(slide_imgs, PPTX_OUT)
    finally:
        if tmp_dir.exists():
